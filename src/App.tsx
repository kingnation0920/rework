...
const ROUTE_META: Record<string, { title: string; description: string }> = {
  ...
  '/blog/ai-era-talent-evaluation-criteria': {
    title: 'AI 시대의 새로운 인재 평가 기준 | 랩리워크 블로그',
...
const BLOG_POSTS: BlogPost[] = [
  {
    slug: 'ai-era-talent-evaluation-criteria',
    title: 'AI 시대의 새로운 인재 평가 기준을 어떻게 설계할까?',
    description: '생성형 AI의 보편화로 기존의 텍스트나 코드 양 중심의 성과 평가는 더 이상 작동하지 않습니다.',
    category: 'AI HRD',
    date: '2026-07-01',
    readTime: '7분',
    tags: ['AI 시대 인재평가', '성과관리', 'AI 리터러시', '인사평가', '업무재설계', 'HRD', '생성형 AI', '리더십'],
    paragraphs: [
      '생성형 AI의 보편화로 기존의 텍스트나 코드 양 중심의 성과 평가는 더 이상 작동하지 않습니다. 미래형 고성과자는 AI를 도구로 다루는 능력과 결과물을 비판적으로 검증하는 AI 리터러시를 갖춘 사람입니다. 결과 중심에서 업무 재설계 과정 중심의 다면적 인재 평가 프레임워크로의 전환이 필요합니다.',
      ...
      'AI 시대의 새로운 인재 평가 기준이란 단순히 완성된 결과물의 물리적 부피를 측정하는 전통적 방식을 넘어, 구성원이 인공지능과 협업하며 문제를 정의하고 도출된 결과물을 비판적으로 조율해 나가는 과정 전반을 다면적으로 평가하는 체계를 말합니다.',
      'AI가 만든 결과물의 홍수 속에서 길을 잃은 리더들',
      '정확한 회사명과 세부 조건은 밝힐 수 없지만, 최근 여러 프로젝트에서 반복해서 본 장면은 분명합니다. 과거에는 기획안 서두의 시장 분석을 정교하게 채우거나 기초적인 분석 리포트를 남들보다 빠르게 만드는 직원이 유능하다는 평가를 받았습니다. 그러나 생성형 AI가 단 몇 초 만에 그럴듯한 초안을 뽑아내면서 상황이 완전히 바뀌었습니다. 모든 직원의 서류가 상향 평준화된 것처럼 보이기 시작한 것입니다.',
      '리더들은 겉보기에 완벽한 보고서들 사이에서 진짜 문제를 해결한 인재가 누구인지 구별해 내지 못해 랩리워크에 컨설팅을 요청해 오셨습니다. 한 유통 서비스 중견기업의 인사 담당자는 기존 평가 지표로는 생성형 AI를 몰래 써서 양만 채운 결과물과 진지하게 고민해 만든 결과물을 도저히 걸러낼 수 없다고 고백하기도 했습니다.',
      '양적 산출물에서 질문과 검증의 프로세스로 일의 흐름 재설계하기',
      '제가 이 문제를 볼 때 중요하게 보는 건 도구의 사용법이 아니라 일하는 방식과 평가 기준의 싱크로율입니다. 생성형 AI가 업무에 깊숙이 들어온 조직일수록, 평가는 결과 중심에서 과정 중심으로 이동해야 합니다. 랩리워크는 현장에서 이 지점을 단순한 인사 제도 개편으로만 풀지 않고 업무 재설계라는 렌즈로 접근합니다.',
      'AI 시대의 고성과자는 단순히 기술을 잘 다루는 사람이 아닙니다. 해결해야 할 비즈니스 문제를 정확하게 쪼개어 AI에게 명확하게 질문하는 프롬프팅 역량, 그리고 AI가 내놓은 그럴듯한 환각 현상을 필터링하는 비판적 검증 능력을 동시에 가진 사람입니다. 이 두 가지 역량이 결합된 상태를 우리는 AI 리터러시라고 부르며, 이것이야말로 앞으로 조직이 주목해야 할 새로운 핵심 역량 지표가 됩니다.',
      '평가 행동이 바뀌려면 리더가 주니어의 최종 보고서만 볼 것이 아니라, 그 보고서를 도출하기 위해 어떤 가설을 세웠고 AI와 어떤 피드백 루프를 돌렸는지 중간 과정을 관찰할 수 있도록 업무의 흐름을 다시 짜야 합니다.',
      '데이터와 글로벌 연구가 증명하는 고성과자의 재정의',
      '실제 글로벌 인사 데이터와 학술 연구에서도 이러한 변화의 필요성이 뚜렷하게 관찰됩니다. 글로벌 인사 성과 플랫폼 기업 Betterworks가 발표한 2026년 성과 역량 보고서에 따르면, 조사에 참여한 인사 조직 리더의 90%가 인공지능이 이미 고성과자의 기준을 완전히 재정의했다고 응답했습니다. 그러나 정작 AI 시대의 새로운 인재 평가 기준을 조직의 공식적인 평가 제도로 업데이트했다고 답한 기업은 42%에 불과해 극심한 격차를 보여줍니다.',
      '또한 Human Resource Management에 게재된 Pan, Froese, Xue의 연구는 조직 내부에서 AI와 인간의 협업 관계를 명확히 정립하고 평가의 다면적 신뢰성을 확보할 때 구성원의 인사 평가 만족도와 조직 몰입도를 높이는 실무적 단서를 제공합니다. 도구의 도입 속도를 인간의 평가 제도가 따라가지 못할 때 조직 내부의 불신과 저항이 생긴다는 실증적 근거입니다.',
      '그래서 저는 이런 고객사를 만날 때마다 도구 사용법을 가르치는 일회성 강의 대신, 리더와 구성원이 함께 모여 평가 지표를 합의하는 워크숍을 제안합니다. 랩리워크는 이 지점을 교육으로만 풀지 않습니다. 실제로 제조, 서비스, IT 등 다양한 업종을 합성한 컨설팅 현장에서 확인한 핵심은 작은 실험으로 평가 방식을 바꿔야 정착된다는 사실입니다.',
      '예를 들어, 한 분기 동안 주 단위 싱크업 미팅에서 결과물이 아니라 이번 주에 AI와 검증한 가설의 목록을 공유하게 만드는 구조적 실험을 진행했습니다. 이를 통해 리더는 직원이 문제를 정의하는 깊이를 관찰할 수 있게 되었고, 자연스럽게 AI 시대의 새로운 인재 평가 기준을 수립할 수 있었습니다. 시스템을 대대적으로 바꾸기 전에 일상적인 피드백 방식부터 재설계하는 관점의 전환이 필요합니다.',
      '새로운 평가 기준을 도입할 때 주니어 직원들의 변화 저항을 어떻게 관리해야 하나요?',
      'AI 시대의 평가 기준 변경을 상시적인 감시나 감찰로 받아들이지 않도록 심리적 안전감을 주는 것이 우선입니다. 랩리워크는 프로세스 평가의 목적이 통제가 아니라 성장을 돕는 상시 코칭에 있음을 리더의 언어로 명확히 전달하도록 가이드합니다. 결과물의 실패를 자책하기보다 AI와의 협업 과정에서 무엇을 배웠는지 공유하는 문화를 먼저 정착시키는 것이 저항을 줄이는 지름길입니다.',
      '전통적인 정량적 성과 지표와 AI 리터러시 평가 지표의 가장 결정적인 차이는 무엇인가요?',
      '가장 큰 차이는 평가 대상이 산출물의 결과인가 아니면 문제를 해결해 나가는 판단력과 맥락 이해도인가에 있습니다. 기존 지표가 정해진 시간 동안 몇 개의 문서를 썼는지에 집착했다면, 새로운 지표는 AI가 제안한 대안 중 우리 비즈니스 상황에 맞는 최적의 안을 골라낸 이유와 비판적 검증의 흔적을 평가합니다. 즉, 단순 노동의 양이 아니라 인간 고유의 의사결정 강도를 측정하는 데 본질적인 차이가 있습니다.',
      '랩리워크는 새로운 파도가 지형을 바꾸는 지금, 우리의 평가 기준이 여전히 과거의 문법에 고여 있지 않은지 끊임없이 질문을 던지며 일하는 방식의 진짜 변화를 만들어 가겠습니다.'
    ],
    takeaways: [
    ...
    sources: [
      'Betterworks. State of Performance Enablement 2026 Report. Betterworks Insights. 2026.',
      'Pan Y, Froese FJ, Xue S. The Role of AI in Performance Appraisal: A Mixed-Method Study of Employee Experience Through a Relational Lens. Human Resource Management. 2025. doi:10.1002/hrm.70049.'
    ]
  },
  {
